...
def lonlatplot(data=None, lon=None, lat=None, **kw):
    ...
    **** Returns ****
    ...
    m = Basemap(projection=proj, lon_0=lon_0, lat_0=lat_0, boundinglat=boundinglat,
                round=basemap_round, **basemap_kw)
    ...
    if fill_continents:
        ...
        m.fillcontinents(color=continents_color, lake_color=continents_color,
                         **continents_kw)
        ##########+++++++++++++++###########
        # m.drawmapboundary(fill_color='coral')
        m.drawmapboundary(fill_color=continents_color)
        ##########+++++++++++++++###########
    else:
        coastlines_kw = kw.pop('coastlines_kw', {})
        coastlines_color = kw.pop('coastlines_color', '0.5')
    ...
    if lon is None:
        ...
        if not np.isclose(lon_0, (lon[0] + lon[-1]) / 2.0) and proj in ('moll', 'cyl', 'robin'):
    ...
    # cmap.set_bad(color='red', alpha=1.)
    # cmap.set_bad(color='white')
    cmap = plot_kw.pop('cmap', cmap)
    if cmap is None:
        zz_max = data.max()
        zz_min = data.min()
        if zz_min >= 0:
    ...
    if plot_type in ('pcolor',):
        plot_obj = m.pcolor(X_edge, Y_edge, data, cmap=cmap, **plot_kw)
    # pcolormesh
    elif plot_type in ('pcolormesh',):
        plot_obj = m.pcolormesh(X_edge, Y_edge, data, cmap=cmap, **plot_kw)
    # imshow
        ...
        origin = plot_kw.pop('origin', origin)
        extent = kw.pop('extend', [X_edge[0, 0], X_edge[0, -1], Y_edge[0, 0], Y_edge[-1, 0]])
        extent = plot_kw.pop('extent', extent)
        interpolation = kw.pop('interpolation', 'nearest')
        interpolation = plot_kw.pop('interpolation', interpolation)
        plot_obj = m.imshow(data, origin=origin, cmap=cmap, extent=extent,
                            interpolation=interpolation, **plot_kw)
    # contourf
        ...
        # cmap00 = plt.get_cmap('jet')

        plot_obj = m.contourf(X, Y, data,
                              extend=extend,
                              colors=colors,
                              levels=levels,
                              # norm=norm,
    ...
    # contour
        ...
        colors = kw.pop('colors', None)
        colors = plot_kw.pop('colors', colors)
        if colors is not None:
            cmap = None
        plot_obj = m.contour(X, Y, data, cmap=cmap, colors=colors, **plot_kw)
        ...
        quiver_color = plot_kw.pop('color', quiver_color)
        quiver_scale = kw.pop('scale', None)
        quiver_scale = plot_kw.pop('scale', quiver_scale)
        plot_obj = m.quiver(X_, Y_, u_rot, v_rot, color=quiver_color,
                            scale=quiver_scale, **plot_kw)
    ...
    # scatter
        ...
        marker_color = plot_kw.pop('color', marker_color)
        plot_obj = m.scatter(X[L], Y[L], color=marker_color, **plot_kw)
    else:
    ...
    if plot_type in ('pcolor', 'pcolormesh', 'contourf', 'imshow'):
        ...
        cbar_ticks = kw.pop('cbar_ticks', None)
        cbar_ticklabels = kw.pop('cbar_ticklabels', None)

        ax_current = plt.gca()
        divider = make_axes_locatable(ax_current)
        if cbar_type in ('v', 'vertical'):
            # cbar = vcolorbar(units=units)
            cbar_size = kw.pop('cbar_size', '2.5%')
            cbar_size = cbar_kw.pop('size', cbar_size)
            cbar_pad = kw.pop('cbar_pad', 0.05)
            cbar_pad = cbar_kw.pop('pad', cbar_pad)

            cax = divider.append_axes('right', size=cbar_size, pad=cbar_pad)
            cbar = plt.colorbar(cax=cax, extend=cbar_extend,
                                orientation='vertical', ticks=cbar_ticks, **cbar_kw)
            if units is not None and units != '':
                cbar.ax.yaxis.set_label_position('right')
                cbar.ax.set_ylabel(units)

            if cbar_ticklabels is not None:
                cbar.ax.yaxis.set_ticklabels(cbar_ticklabels)
            ...
            cbar = plt.colorbar(cax=cax, extend=cbar_extend,
                                orientation='horizontal', ticks=cbar_ticks, **cbar_kw)
            if units is not None and units != '':
                # cbar.ax.xaxis.set_label_position('bottom')
                cbar.ax.set_xlabel(units)

            if cbar_ticklabels is not None:
                cbar.ax.xaxis.set_ticklabels(cbar_ticklabels)
        plt.sca(ax_current)
